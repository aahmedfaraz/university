import React from 'react';

const Assign2 = () => {
  return (
    <>
      
    {/* <!-- Top Heading --> */}
            <h1>Web Engineering Assignment # 03</h1>






















            {/* <!-- AHMED FARAZ card starts from here --------------------------------------------------------------------------------------------------------------> */}
            {/* <!-- ============================================================= --> */}
            {/* <!-- Activity Card - Group 3 and 6 --> */}
            {/* <!-- ============================================================= --> */}
            <ul class="activity-card" id="group-3-and-6">
                {/* <!-- Site GitHub Link --> */}
                <li class="member self">
                    <span>Ahmed Faraz</span>
                    <span>(02-131192-037)</span>
                    <a target="_blank" rel="noreferrer" href="https://github.com/aahmedfaraz/university/tree/main/university-react-app" class="btn dark">
                        Github
                        <i class="material-icons">code</i>
                    </a>
                    <a target="_blank" rel="noreferrer" href="https://ahmedfaraz-university.netlify.app/" class="btn blue">
                        Live App
                        <i class="material-icons">open_in_new</i>
                    </a>
                </li>
                {/* <!-- GROUP - 3 --> */}
                <li class="activity-heading">
                    <span>Group - 3</span>
                </li>
                {/* <!-- member - 01 --> */}
                <li class="member lead">
                    <span>Muhammad Umer Farooque</span>
                    <span>(02-131192-059)</span>
                    <a target="_blank" rel="noreferrer" href="https://github.com/muhammadumer13/Web-Assignment-3" class="btn dark">
                        Github
                        <i class="material-icons">code</i>
                    </a>
                    <a target="_blank" rel="noreferrer" href="https://spontaneous-panda-1b9235.netlify.app/" class="btn blue">
                        Live App
                        <i class="material-icons">open_in_new</i>
                    </a>
                </li>
                {/* <!-- member - 02 --> */}
                <li class="member">
                    <span>Abdul Basit Khan</span>
                    <span>(02-131192-066)</span>
                    <a target="_blank" rel="noreferrer" href="http://github.com/Basit03/Web-Engineering-Assignment-03" class="btn dark">
                        Github
                        <i class="material-icons">code</i>
                    </a>
                    <a target="_blank" rel="noreferrer" href="http://basit-react-assignment-03.netlify.app/" class="btn blue">
                        Live App
                        <i class="material-icons">open_in_new</i>
                    </a>
                </li>
                {/* <!-- member - 03 --> */}
                <li class="member">
                    <span>Rida Afzal</span>
                    <span>(02-131192-008)</span>
                    <a target="_blank" rel="noreferrer" href="https://github.com/Ridaafzal282/reactassignment" class="btn dark">
                        Github
                        <i class="material-icons">code</i>
                    </a>
                    <a target="_blank" rel="noreferrer" href="https://rida-afzal-assign-3.netlify.app/" class="btn blue">
                        Live App
                        <i class="material-icons">open_in_new</i>
                    </a>
                </li>
                {/* <!-- member - 04 --> */}
                <li class="member">
                    <span>Rimya Fareed</span>
                    <span>(02-131192-010)</span>
                    <a target="_blank" rel="noreferrer" href="https://github.com/RIMYA/WebEngineering3" class="btn dark">
                        Github
                        <i class="material-icons">code</i>
                    </a>
                    <a target="_blank" rel="noreferrer" href="https://benevolent-mochi-07cc6c.netlify.app/" class="btn blue">
                        Live App
                        <i class="material-icons">open_in_new</i>
                    </a>
                </li>
                {/* <!-- GROUP - 6 --> */}
                <li class="activity-heading">
                    <span>Group - 6</span>
                </li>
                {/* <!-- member - 01 --> */}
                <li class="member lead">
                    <span>M Saim Hassan</span>
                    <span>(02-131192-015)</span>
                    <a target="_blank" rel="noreferrer" href="https://github.com/saimhassan28/My-React-App" class="btn dark">
                        Github
                        <i class="material-icons">code</i>
                    </a>
                    <a target="_blank" rel="noreferrer" href="https://peaceful-malasada-2c95dc.netlify.app/" class="btn blue">
                        Live App
                        <i class="material-icons">open_in_new</i>
                    </a>
                </li>
                {/* <!-- member - 02 --> */}
                <li class="member">
                    <span>Muhammad Bilal</span>
                    <span>(02-131192-080)</span>
                    <a target="_blank" rel="noreferrer" href="https://github.com/BilalFareed188/WE-Assignment-03" class="btn dark">
                        Github
                        <i class="material-icons">code</i>
                    </a>
                    <a target="_blank" rel="noreferrer" href="https://web-assignment-03-080.netlify.app/" class="btn blue">
                        Live App
                        <i class="material-icons">open_in_new</i>
                    </a>
                </li>
                {/* <!-- member - 03 --> */}
                <li class="member">
                    <span>Rabiya Essani</span>
                    <span>(02-131192-081)</span>
                    <a target="_blank" rel="noreferrer" href="https://github.com/Rabyaessani/Web-Assignment-3" class="btn dark">
                        Github
                        <i class="material-icons">code</i>
                    </a>
                    <a target="_blank" rel="noreferrer" href="https://rabya-essani-assig-3.netlify.app/" class="btn blue">
                        Live App
                        <i class="material-icons">open_in_new</i>
                    </a>
                </li>
                {/* <!-- member - 04 --> */}
                <li class="member">
                    <span>Saad Kamal</span>
                    <span>(02-131192-075)</span>
                    <a target="_blank" rel="noreferrer" href="/assignment-3" class="btn dark">
                        Github
                        <i class="material-icons">code</i>
                    </a>
                    <a target="_blank" rel="noreferrer" href="/assignment-3" class="btn blue">
                        Live App
                        <i class="material-icons">open_in_new</i>
                    </a>
                </li>
            </ul>
            {/* <!-- AHMED FARAZ card ends here --------------------------------------------------------------------------------------------------------------> */}

























            {/* <!-- MUHAMMAD UMER ADEEM card starts from here --------------------------------------------------------------------------------------------------------------> */}
            {/* <!-- ============================================================= --> */}
            {/* <!-- Activity Card - Group 7 and 1 --> */}
            {/* <!-- ============================================================= --> */}
            <ul class="activity-card" id="group-7-and-1">
                {/* <!-- Site GitHub Link --> */}
                <li class="member self">
                    <span>Muhammad Umer Adeem</span>
                    <span>(02-131192-044)</span>
                    <a target="_blank" rel="noreferrer" href="https://github.com/UmerAdeemCheema/Web-Engineering-Labs/tree/main/Assignment%203/assignment-3" class="btn dark">
                        Github
                        <i class="material-icons">code</i>
                    </a>
                    <a target="_blank" rel="noreferrer" href="https://luminous-wisp-a9769e.netlify.app/" class="btn blue">
                        Live App
                        <i class="material-icons">open_in_new</i>
                    </a>
                </li>
                {/* <!-- GROUP - 7 --> */}
                <li class="activity-heading">
                    <span>Group - 7</span>
                </li>
                {/* <!-- member - 01 --> */}
                <li class="member lead">
                    <span>Adeenah Akhtar</span>
                    <span>(02-131192-024)</span>
                    <a target="_blank" rel="noreferrer" href="https://github.com/AdeenaAkhtar/Assignment03" class="btn dark">
                        Github
                        <i class="material-icons">code</i>
                    </a>
                    <a target="_blank" rel="noreferrer" href="https://adeena-web-engineering-assignment03.netlify.app" class="btn blue">
                        Live App
                        <i class="material-icons">open_in_new</i>
                    </a>
                </li>
                {/* <!-- member - 02 --> */}
                <li class="member">
                    <span>Haris Zahid</span>
                    <span>(02-131192-001)</span>
                    <a target="_blank" rel="noreferrer" href="https://github.com/haris-7523/Web-Assignment-3/tree/main/my-app/my-app" class="btn dark">
                        Github
                        <i class="material-icons">code</i>
                    </a>
                    <a target="_blank" rel="noreferrer" href="https://luminous-bienenstitch-b7d257.netlify.app/" class="btn blue">
                        Live App
                        <i class="material-icons">open_in_new</i>
                    </a>
                </li>
                {/* <!-- member - 03 --> */}
                <li class="member">
                    <span>Saba Sunawar</span>
                    <span>(02-131192-009)</span>
                    <a target="_blank" rel="noreferrer" href="https://github.com/sabasunawar/React-Assignment-03" class="btn dark">
                        Github
                        <i class="material-icons">code</i>
                    </a>
                    <a target="_blank" rel="noreferrer" href="https://effortless-marzipan-2a6e1c.netlify.app/" class="btn blue">
                        Live App
                        <i class="material-icons">open_in_new</i>
                    </a>
                </li>
                {/* <!-- member - 04 --> */}
                <li class="member">
                    <span>Sami Ullah Khan</span>
                    <span>(02-131192-071)</span>
                    <a target="_blank" rel="noreferrer" href="https://github.com/samikhan456/Assignment-3-Web-React" class="btn dark">
                        Github
                        <i class="material-icons">code</i>
                    </a>
                    <a target="_blank" rel="noreferrer" href="https://profound-crostata-373fab.netlify.app" class="btn blue">
                        Live App
                        <i class="material-icons">open_in_new</i>
                    </a>
                </li>
                {/* <!-- GROUP - 1 --> */}
                <li class="activity-heading">
                    <span>Group - 1</span>
                </li>
                <li class="member lead">
                    <span>Muhammad Sohail Hameed</span>
                    <span>(02-131192-083)</span>
                    <a target="_blank" rel="noreferrer" href="https://github.com/MuhammadSohail083/WebEngAssignment-3.git" class="btn dark">
                        Github
                        <i class="material-icons">code</i>
                    </a>
                    <a target="_blank" rel="noreferrer" href="https://silver-bonbon-5d74e4.netlify.app" class="btn blue">
                        Live App
                        <i class="material-icons">open_in_new</i>
                    </a>
                </li>
                {/* <!-- member - 02 --> */}
                <li class="member">
                    <span>Raja Humza Nawaz</span>
                    <span>(02-131192-072)</span>
                    <a target="_blank" rel="noreferrer" href="https://github.com/RajaHumzaNawaz/Web-Development-Labs/tree/main/Web%20Assignment%203" class="btn dark">
                        Github
                        <i class="material-icons">code</i>
                    </a>
                    <a target="_blank" rel="noreferrer" href="https://helpful-griffin-7eceb7.netlify.app/" class="btn blue">
                        Live App
                        <i class="material-icons">open_in_new</i>
                    </a>
                </li>
                {/* <!-- member - 03 --> */}
                <li class="member">
                    <span>Sannia Amin</span>
                    <span>(02-131192-068)</span>
                    <a target="_blank" rel="noreferrer" href="https://github.com/Sannia-Amin-Anjum/MY_REACT_SITE" class="btn dark">
                        Github
                        <i class="material-icons">code</i>
                    </a>
                    <a target="_blank" rel="noreferrer" href="https://meek-tarsier-3e526d.netlify.app/" class="btn blue">
                        Live App
                        <i class="material-icons">open_in_new</i>
                    </a>
                </li>
                {/* <!-- member - 04 --> */}
                <li class="member">
                    <span>Amna Nadeem</span>
                    <span>(02-131192-003)</span>
                    <a target="_blank" rel="noreferrer" href="https://github.com/AmnaNadeem999/MyWEAssignment3" class="btn dark">
                        Github
                        <i class="material-icons">code</i>
                    </a>
                    <a target="_blank" rel="noreferrer" href="https://amna-ka-react-app.netlify.app/" class="btn blue">
                        Live App
                        <i class="material-icons">open_in_new</i>
                    </a>
                </li>
            </ul>
            {/* <!-- MUHAMMAD UMER ADEEM card ends here --------------------------------------------------------------------------------------------------------------> */}






















 {/* <!-- HAMZA BUKHARI card starts from here --------------------------------------------------------------------------------------------------------------> */}
            {/* <!-- ============================================================= --> */}
            {/* <!-- Activity Card - Group 5 and 4 --> */}
            {/* <!-- ============================================================= --> */}
            <ul class="activity-card" id="group-5-and-4">
                {/* <!-- Site GitHub Link --> */}
                <li class="member self">
                    <span>Hamza Bukhari</span>
                    <span>(02-131192-022)</span>
                    <a target="_blank" rel="noreferrer" href="https://github.com/SMHamzaBukhari/Web-Eng-Assignment-3.git" class="btn dark">
                        Github
                        <i class="material-icons">code</i>
                    </a>
                    <a target="_blank" rel="noreferrer" href="https://web-assign-737fe.web.app/" class="btn blue">
                        Live App
                        <i class="material-icons">open_in_new</i>
                    </a>
                </li>
                {/* <!-- GROUP - 5 --> */}
                <li class="activity-heading">
                    <span>Group - 5</span>
                </li>
                <li class="member lead">
                    <span>Ahmed Bin Alam Rao</span>
                    <span>(02-131192-046)</span>
                    <a target="_blank" rel="noreferrer" href="/assignment-3" class="btn dark">
                        Github
                        <i class="material-icons">code</i>
                    </a>
                    <a target="_blank" rel="noreferrer" href="https://app.netlify.com/sites/joyful-custard-e9551f/overview" class="btn blue">
                        Live App
                        <i class="material-icons">open_in_new</i>
                    </a>
                </li>
                {/* <!-- member - 02 --> */}
                <li class="member">
                    <span>Hareem Fatima</span>
                    <span>(02-131192-020)</span>
                    <a target="_blank" rel="noreferrer" href="https://github.com/hareem19599/WebAssignments/tree/main/assignment3" class="btn dark">
                        Github
                        <i class="material-icons">code</i>
                    </a>
                    <a target="_blank" rel="noreferrer" href="https://spectacular-dasik-169d90.netlify.app/" class="btn blue">
                        Live App
                        <i class="material-icons">open_in_new</i>
                    </a>
                </li>
                {/* <!-- member - 03 --> */}
                <li class="member">
                    <span>Abdul Rehman</span>
                    <span>(02-131192-064)</span>
                    <a target="_blank" rel="noreferrer" href="https://abdulrahman383.github.io/Web-Assignment-3/" class="btn dark">
                        Github
                        <i class="material-icons">code</i>
                    </a>
                    <a target="_blank" rel="noreferrer" href="https://abdul-rehman-assign-3.netlify.app/" class="btn blue">
                        Live App
                        <i class="material-icons">open_in_new</i>
                    </a>
                </li>
                {/* <!-- member - 04 --> */}
                <li class="member">
                    <span>Sadia Ambreen</span>
                    <span>(02-131192-056)</span>
                    <a target="_blank" rel="noreferrer" href="https://github.com/sadiaambreen/WebAssignment3" class="btn dark">
                        Github
                        <i class="material-icons">code</i>
                    </a>
                    <a target="_blank" rel="noreferrer" href="https://sadia59.netlify.app/" class="btn blue">
                        Live App
                        <i class="material-icons">open_in_new</i>
                    </a>
                </li>
                {/* <!-- GROUP - 4 --> */}
                <li class="activity-heading">
                    <span>Group - 4</span>
                </li>
                <li class="member lead">
                    <span>Abdullah Iqbal</span>
                    <span>(02-131192-076)</span>
                    <a target="_blank" rel="noreferrer" href="https://github.com/AbdullahIqbal10/Assignment-3-react" class="btn dark">
                        Github
                        <i class="material-icons">code</i>
                    </a>
                    <a target="_blank" rel="noreferrer" href="https://abdullahiqbalweb3.netlify.app/" class="btn blue">
                        Live App
                        <i class="material-icons">open_in_new</i>
                    </a>
                </li>
                {/* <!-- member - 02 --> */}
                <li class="member">
                    <span>Arzoo Fatima</span>
                    <span>(02-131192-032)</span>
                    <a target="_blank" rel="noreferrer" href="https://github.com/arzoofatima52/Web-Assignment-3" class="btn dark">
                        Github
                        <i class="material-icons">code</i>
                    </a>
                    <a target="_blank" rel="noreferrer" href="https://arzooassignment3.firebaseapp.com/" class="btn blue">
                        Live App
                        <i class="material-icons">open_in_new</i>
                    </a>
                </li>
                {/* <!-- member - 03 --> */}
                <li class="member">
                    <span>Shahwaiz Hassan</span>
                    <span>(02-131192-006)</span>
                    <a target="_blank" rel="noreferrer" href="https://github.com/shahwaiz-hassan/React-Assignment" class="btn dark">
                        Github
                        <i class="material-icons">code</i>
                    </a>
                    <a target="_blank" rel="noreferrer" href="https://shahwaizhassan-react.netlify.app/" class="btn blue">
                        Live App
                        <i class="material-icons">open_in_new</i>
                    </a>
                </li>
                {/* <!-- member - 04 --> */}
                <li class="member">
                    <span>Arshia Naveed</span>
                    <span>(02-131192-026)</span>
                    <a target="_blank" rel="noreferrer" href="https://github.com/ArshiaNaveed/Assignment03.git" class="btn dark">
                        Github
                        <i class="material-icons">code</i>
                    </a>
                    <a target="_blank" rel="noreferrer" href="https://arshianaveededitingwebsite.netlify.app/" class="btn blue">
                        Live App
                        <i class="material-icons">open_in_new</i>
                    </a>
                </li>
            </ul>
            {/* <!-- HAMZA BUKHARI card ends here --------------------------------------------------------------------------------------------------------------> */}




            














            {/* <!-- ADIL WAHEED card starts from here --------------------------------------------------------------------------------------------------------------> */}
            {/* <!-- ============================================================= --> */}
            {/* <!-- Activity Card - Group 2 and 8 --> */}
            {/* <!-- ============================================================= --> */}
            <ul class="activity-card" id="group-2-and-8">
                {/* <!-- Site GitHub Link --> */}
                <li class="member self">
                    <span>Adil Waheed</span>
                    <span>(02-131192-082)</span>
                    <a target="_blank" rel="noreferrer" href="https://github.com/adilwaheed786/Portfolio/tree/main/React" class="btn dark">
                        Github
                        <i class="material-icons">code</i>
                    </a>
                    <a target="_blank" rel="noreferrer" href="https://adilwaheed.netlify.app/" class="btn blue">
                        Live App
                        <i class="material-icons">open_in_new</i>
                    </a>
                </li>
                {/* <!-- GROUP - 2 --> */}
                <li class="activity-heading">
                    <span>Group - 2</span>
                </li>
                <li class="member lead">
                    <span>Syed Muhammad Shoaib</span>
                    <span>(02-131192-030)</span>
                    <a target="_blank" rel="noreferrer" href="https://github.com/Shah-1510/Assignment-3-Portfolio.git" class="btn dark">
                        Github
                        <i class="material-icons">code</i>
                    </a>
                    <a target="_blank" rel="noreferrer" href="https://portfolioreactassignment.netlify.app/" class="btn blue">
                        Live App
                        <i class="material-icons">open_in_new</i>
                    </a>
                </li>
                {/* <!-- member - 02 --> */}
                <li class="member">
                    <span>Dania Ahmed</span>
                    <span>(02-131192-079)</span>
                    <a target="_blank" rel="noreferrer" href="https://github.com/Dania722/Explore_react-website_github" class="btn dark">
                        Github
                        <i class="material-icons">code</i>
                    </a>
                    <a target="_blank" rel="noreferrer" href="https://reactsite-2f7e2.web.app/" class="btn blue">
                        Live App
                        <i class="material-icons">open_in_new</i>
                    </a>
                </li>
                {/* <!-- member - 03 --> */}
                <li class="member">
                    <span>Sajjad Ahmed</span>
                    <span>(02-131192-052)</span>
                    <a target="_blank" rel="noreferrer" href="https://github.com/sajjadkhattakahmed/webTasks/tree/main/Assignments/a3" class="btn dark">
                        Github
                        <i class="material-icons">code</i>
                    </a>
                    <a target="_blank" rel="noreferrer" href="https://kaleidoscopic-biscochitos-97ab9f.netlify.app/" class="btn blue">
                        Live App
                        <i class="material-icons">open_in_new</i>
                    </a>
                </li>
                {/* <!-- member - 04 --> */}
                <li class="member">
                    <span>Summayya Khalid</span>
                    <span>(02-131192-057)</span>
                    <a target="_blank" rel="noreferrer" href="https://github.com/sumayyakhalid/foodweb3" class="btn dark">
                        Github
                        <i class="material-icons">code</i>
                    </a>
                    <a target="_blank" rel="noreferrer" href="https://assingment3web.netlify.app/" class="btn blue">
                        Live App
                        <i class="material-icons">open_in_new</i>
                    </a>
                </li>
                {/* <!-- GROUP - 8 --> */}
                <li class="activity-heading">
                    <span>Group - 8</span>
                </li>
                <li class="member lead">
                    <span>Marium Ahmed</span>
                    <span>(02-131192-014)</span>
                    <a target="_blank" rel="noreferrer" href="/assignment-3" class="btn dark">
                        Github
                        <i class="material-icons">code</i>
                    </a>
                    <a target="_blank" rel="noreferrer" href="/assignment-3" class="btn blue">
                        Live App
                        <i class="material-icons">open_in_new</i>
                    </a>
                </li>
                {/* <!-- member - 02 --> */}
                <li class="member">
                    <span>Mirza Muhammad Farooq Baig</span>
                    <span>(02-131192-040)</span>
                    <a target="_blank" rel="noreferrer" href="https://github.com/farooqmirza5/WEB_ASSIGNMENT_3" class="btn dark">
                        Github
                        <i class="material-icons">code</i>
                    </a>
                    <a target="_blank" rel="noreferrer" href="https://farooqmirza5.github.io/WEB_ASSIGNMENT_3/" class="btn blue">
                        Live App
                        <i class="material-icons">open_in_new</i>
                    </a>
                </li>
                {/* <!-- member - 03 --> */}
                <li class="member">
                    <span>Ahmed Faraz Ali</span>
                    <span>(02-131192-023)</span>
                    <a target="_blank" rel="noreferrer" href="https://github.com/AhmedFarazAli/Assignment-3" class="btn dark">
                        Github
                        <i class="material-icons">code</i>
                    </a>
                    <a target="_blank" rel="noreferrer" href="https://ahmedfarazali.github.io/Assignment-3/" class="btn blue">
                        Live App
                        <i class="material-icons">open_in_new</i>
                    </a>
                </li>
                {/* <!-- member - 04 --> */}
                <li class="member">
                    <span>Muhammad Shazil Bin Alam Rao</span>
                    <span>(02-131192-047)</span>
                    <a target="_blank" rel="noreferrer" href="https://github.com/shazilrao/WEA3" class="btn dark">
                        Github
                        <i class="material-icons">code</i>
                    </a>
                    <a target="_blank" rel="noreferrer" href="https://cosmic-longma-5515a2.netlify.app/" class="btn blue">
                        Live App
                        <i class="material-icons">open_in_new</i>
                    </a>
                </li>
            </ul>
            {/* <!-- ADIL WAHEED card ends here --------------------------------------------------------------------------------------------------------------> */}




























    </>
  )
}

export default Assign2;
